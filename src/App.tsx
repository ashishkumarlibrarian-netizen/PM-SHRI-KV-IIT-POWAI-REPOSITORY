...
import { 
  BookOpen, Sparkles, Award, MessageSquare, Heart, Send, 
  Volume2, VolumeX, RefreshCw, Search, Compass, ChevronRight, 
  Bookmark, MapPin, School, Hash, User, CheckCircle2, 
  ArrowRight, PenTool, HelpCircle, Calendar, Flame, LayoutGrid, 
  Maximize2, BookOpenCheck, ExternalLink, Moon, Sun, Library,
  Instagram, Youtube, Twitter, Facebook, Users, Radio, Bell
} from "lucide-react";
import WelcomeTab from "./components/WelcomeTab";
import { StoryChapter, BookRecommendation, SocialFeedPost, SavedStory, User as UserType, RecentBook } from "./types";
...
export default function App() {
  const [activeTab, setActiveTab] = useState<"dashboard" | "story" | "books" | "creative" | "social">("dashboard");
  ...
  const [storyHistory, setStoryHistory] = useState<{ segment: string; choiceMade?: string }[]>([]);
  ...
  const [recentBooksList, setRecentBooksList] = useState<RecentBook[]>(RECENT_BOOKS_DATA);
  const [booksSearchQuery, setBooksSearchQuery] = useState("");
  const [selectedGenreFilter, setSelectedGenreFilter] = useState("All");
  const [selectedGradeFilter, setSelectedGradeFilter] = useState("All");
  const [selectedStatusFilter, setSelectedStatusFilter] = useState("All");
  const [savedFavoritesMap, setSavedFavoritesMap] = useState<Record<string, boolean>>({});
  const [bookReservationSuccess, setBookReservationSuccess] = useState<string | null>(null);
  ...
  const [creativeKeywords, setCreativeKeywords] = useState("IIT gates, raindrops, books, discovery");
  const [creativeResult, setCreativeResult] = useState<{title: string, output: string, educationalTips: string[]} | null>(null);
  const [isCreativeLoading, setIsCreativeLoading] = useState(false);

  // Social Feed Simulation
  const [socialPosts, setSocialPosts] = useState<SocialFeedPost[]>(INITIAL_SOCIAL_POSTS);
  ...
  const [postCommentsMap, setPostCommentsMap] = useState<Record<string, string[]>>({});
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && window.speechSynthesis) {
      ...
      const updateVoices = () => {
        ...
        const defaultVoice = voices.find(v => v.lang.includes("en-IN") || v.lang.includes("en-US")) || voices[0];
  ...
  const handleStartAdventure = async () => {
    ...
    try {
      ...
      setCurrentChapter({
        ...
        illustrationPrompt: "A cozy high school library corner in Mumbai, glowing blue book, tropical palms outside, sunset cozy light",
        choices: [
  ...
  const handleChoosePath = async (choiceText: string) => {
    ...
    const updatedHistory = [...storyHistory, { segment: currentChapter.storySegment, choiceMade: choiceText }];
    ...
    try {
      ...
      setCurrentChapter({
        title: isFin ? "The Dawn of Cosmic Discovery" : "Deep in the Tech Labyrinth",
        storySegment: isFin 
        ...
        illustrationPrompt: "Water painting of an Indian student holding a shiny digital notebook, background with stars",
        choices: isFin ? [] : [
  ...
  const handleSaveCurrentStory = () => {
    ...
    alert("Superb! Your adventure has been preserved in the Library digital logbook.");
  };
  ...
  const handleReserveBook = (bookId: string) => {
    ...
    if (book.status === "Reserved") {
      setBookReservationSuccess(`This copy of "${book.title}" is already reserved by another reader.`);
      setTimeout(() => setBookReservationSuccess(null), 5000);
      return;
    }
    if (book.status === "Reference Only") {
      setBookReservationSuccess(`"${book.title}" is a Reference-Only book and cannot be checked out outside the library.`);
      setTimeout(() => setBookReservationSuccess(null), 5000);
      return;
    }
    ...
    const borrowerInfo = currentUser ? currentUser.fullName : `${studentName} (Guest)`;
  ...
  const generateCreativeContent = async () => {
    ...
    try {
      const res = await fetch("/api/creative/write", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          topic: creativeTopic,
  ...
  const handleAddPost = (e: React.FormEvent) => {
    ...
    const tagsArray = newPostTags ? newPostTags.split(",").map(t => t.trim().replace(/^#/, "")) : ["KVPowaiReads"];
  ...
  const handleToggleFollow = (platform: string, handleName: string) => {
    ...
    if (!isCurrentlyFollowing) {
      setSocialAlertMessage(`🎉 Hurrah! You are now subscribed to PM Shri KV IIT Powai Library updates on ${platform} (${handleName}). New alerts will be notified here.`);
    } else {
  ...
  const filteredRecentBooks = recentBooksList.filter(book => {
    const matchesSearch = book.title.toLowerCase().includes(booksSearchQuery.toLowerCase()) ||
    ...
    const matchesGenre = selectedGenreFilter === "All" || book.genre.includes(selectedGenreFilter);
    
    const matchesGrade = selectedGradeFilter === "All" || 
      (selectedGradeFilter === "Primary" && book.gradeLevel.includes("Primary")) ||
      (selectedGradeFilter === "Middle" && book.gradeLevel.includes("Middle")) ||
      (selectedGradeFilter === "High" && book.gradeLevel.includes("High")) ||
      (selectedGradeFilter === "Secondary" && book.gradeLevel.includes("Secondary"));
  ...
  return (
    <div className={`min-h-screen ${theme === "dark" ? "dark bg-slate-950 text-slate-100" : "bg-[#f8fafc] text-indigo-950"} font-sans flex flex-col antialiased transition-colors duration-300`}>
      
      {/* PM Shri Premium Bento Nav Header */}
      <header id="app-bento-header" className="h-18 sticky top-0 z-40 bg-slate-900 border-b border-indigo-900 text-white shadow-md px-4 md:px-8 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-white overflow-hidden border border-slate-700/50 shadow flex-shrink-0">
            <img 
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold px-2 py-0.5 rounded bg-amber-500/20 text-amber-300 border border-amber-500/30 font-sans tracking-wide">
                PM SHRI SCHOOL
              </span>
              <span className="hidden sm:inline text-[11px] font-mono text-cyan-400">IIT POWAI SECTOR</span>
            </div>
            <h1 className="text-sm md:text-lg font-bold leading-none tracking-tight text-white mt-1">
        ...
        <nav className="hidden lg:flex items-center gap-2 text-xs font-semibold">
          <button 
            ...
            className={`px-3 py-2 rounded-lg transition-all flex items-center gap-1.5 ${
              activeTab === "dashboard" ? "bg-amber-500 text-slate-950 shadow-sm" : "text-slate-300 hover:bg-white/10"
            }`}
          >
          ...
          <button 
            ...
            className={`px-3 py-2 rounded-lg transition-all flex items-center gap-1.5 ${
              activeTab === "story" ? "bg-amber-500 text-slate-950 shadow-sm" : "text-slate-300 hover:bg-white/10"
            }`}
          >
            <Sparkles className="w-4 h-4 text-amber-300 group-hover:animate-pulse" /> AI Interactive Storyteller
          </button>
          
          <button 
            onClick={() => setActiveTab("books")}
            className={`px-3 py-2 rounded-lg transition-all flex items-center gap-1.5 ${
              activeTab === "books" ? "bg-amber-500 text-slate-950 shadow-sm" : "text-slate-300 hover:bg-white/10"
            }`}
          >
          ...
          <button 
            ...
            className={`px-3 py-2 rounded-lg transition-all flex items-center gap-1.5 ${
              activeTab === "creative" ? "bg-amber-500 text-slate-950 shadow-sm" : "text-slate-300 hover:bg-white/10"
            }`}
          >
          ...
          <button 
            ...
            className={`px-3 py-2 rounded-lg transition-all flex items-center gap-1.5 ${
              activeTab === "social" ? "bg-amber-500 text-slate-950 shadow-sm" : "text-slate-300 hover:bg-white/10"
            }`}
          >
        ...
        <div className="flex items-center gap-3">
          {currentUser ? (
            <div className="flex items-center gap-3">
              <div className="hidden md:flex flex-col text-right animate-fade-in">
                <span className="text-xs font-bold text-amber-400">{currentUser.fullName}</span>
                <span className="text-[10px] text-emerald-400 font-mono font-semibold">✓ Registered Student</span>
              </div>
              <button
                onClick={handleLogout}
                id="header-logout-btn"
                className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-750 hover:text-red-400 text-slate-300 rounded-lg text-[11px] font-bold transition-all border border-slate-700 font-sans"
              >
                Sign Out
              </button>
            </div>
            ...
            <div className="flex items-center gap-2">
              <div className="hidden md:flex flex-col text-right">
                <span className="text-xs font-bold text-amber-500 font-sans">Guest Scholar</span>
                <span className="text-[10px] text-slate-400 font-mono">Offline session</span>
              </div>
              <button
                onClick={() => setIsAuthOpen(true)}
                id="header-login-btn"
                className="px-3.5 py-1.5 bg-amber-500 hover:bg-amber-600 active:scale-95 text-slate-950 rounded-xl text-xs font-bold transition-all shadow-md hover:shadow-amber-500/10 hover:border-amber-400 border border-amber-500"
              >
                Log InOrRegister
              </button>
            </div>
          ...
          <button
            ...
            className="p-2.5 bg-slate-800 hover:bg-slate-700 hover:text-amber-300 text-amber-400 rounded-xl border border-slate-700 transition-all active:scale-95 flex items-center justify-center shadow-md cursor-pointer"
          >
            {theme === "dark" ? <Sun className="w-4.5 h-4.5" /> : <Moon className="w-4.5 h-4.5" />}
          </button>
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-amber-400 to-amber-600 flex items-center justify-center font-bold text-slate-900 border border-amber-300">
            🎓
          </div>
        </div>
      </header>
      ...
      <div className="lg:hidden bg-slate-800 text-slate-100 overflow-x-auto whitespace-nowrap scrollbar-none border-b border-indigo-950">
      ...
      <section className="bg-amber-500/5 dark:bg-slate-900/30 border-b border-amber-500/10 dark:border-slate-800 py-2 px-4 md:px-8 text-xs flex flex-wrap gap-4 items-center justify-between">
        <div className="flex items-center gap-2 text-slate-600 dark:text-slate-300">
          {currentUser ? (
            <div className="flex items-center gap-1.5 text-emerald-800 dark:text-emerald-300 font-semibold bg-emerald-50 dark:bg-emerald-950/40 px-2.5 py-1 rounded-full border border-emerald-200 dark:border-emerald-900/50 animate-fade-in">
              ...
              <span>🔒 Connected as student: <strong>{currentUser.fullName}</strong></span>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <span className="font-semibold text-slate-800 dark:text-slate-200">Customize Guest Account:</span>
              <input 
                type="text" 
                value={studentName}
                onChange={(e) => setStudentName(e.target.value)}
                className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded px-2 py-0.5 text-xs text-slate-800 dark:text-slate-100 w-32 focus:outline-none focus:border-amber-500"
                placeholder="Name" 
              />
              <select 
                value={studentClass} 
        ...
        <div className="flex items-center gap-2 text-red-800 dark:text-amber-400 font-medium">
          ...
          <span>PM Shri School digital library infrastructure active (Mumbai Region)</span>
        </div>
      </section>

      {/* Main Body */}
      <main id="main-content-area" className="flex-1 max-w-7xl w-full mx-auto p-4 md:p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-slate-900 text-white rounded-3xl p-6 border border-slate-800 flex flex-col justify-between shadow-lg relative overflow-hidden group">
                  ...
                  <div className="space-y-3 relative z-10">
                    ...
                    <h3 className="text-xl font-bold font-sans text-amber-100">AI Story Adventure</h3>
                    <p className="text-slate-300 text-xs leading-relaxed">
                      Wander deep into classical folklore, Indian heritage, and local Powai science riddles with adaptive story narration.
                    </p>
                  </div>
                  <button 
                    onClick={() => setActiveTab("story")}
                  ...
                  >
                    Enter Story Realm <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />
                  </button>
                </div>
                ...
                <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex flex-col justify-between group">
                  <div className="space-y-4">
                    ...
                    <div className="bg-slate-50/80 p-3.5 rounded-2xl border border-slate-100 space-y-2">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-indigo-500 text-white text-[10px] font-semibold flex items-center justify-center">AK</div>
                        <span className="text-[11px] font-bold text-slate-700">Ashish Kumar (Librarian)</span>
                      </div>
                      <p className="text-slate-600 text-xs italic leading-relaxed">
                        "Delighted with the responses for the book-review contest. Students of KV IIT Powai keep elevating our parameters of discussion!"
                      </p>
                    </div>
                  </div>
                  <button 
                  ...
                  >
                    Go to Library Buzz Feed <ChevronRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" />
                  </button>
                </div>
                ...
                <div className="bg-gradient-to-br from-indigo-800 to-purple-900 text-white rounded-3xl p-6 shadow-md flex flex-col justify-between relative overflow-hidden">
                  <div className="space-y-3">
                    ...
                    <h3 className="text-lg font-bold text-violet-100">How to use "Mentor Tidbits"?</h3>
                    <p className="text-violet-200/90 text-xs leading-relaxed">
                      Type key words in our Creative Hub to generate poems. The integrated AI highlights literary devices like *Alliteration*, *Hyperbole* or *Personification* to sharpen your school essay skills!
                    </p>
                  </div>
                  <button 
                    onClick={() => setActiveTab("creative")}
              ...
              <div className="lg:col-span-4 bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-6">
                <div>
                  <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                    <Sparkles className="w-5 h-5 text-amber-500" /> Story Parameters
                  </h2>
                  <p className="text-xs text-slate-400">Configure your personalized fables</p>
                </div>

                <div className="space-y-4">
                  {/* Genre selection under KV Traditions */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">Traditional Genre / Theme</label>
                    <select 
                      value={storyGenre}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">Protagonist Persona</label>
                    <select
                      value={protagonist}
                    ...
                    <div className="mt-2">
                      <span className="text-[10px] text-slate-400">Or type custom name:</span>
                      <input 
                        type="text" 
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">Reading Age / Tone</label>
                    <div className="grid grid-cols-2 gap-2 text-center text-xs">
                      {["Juniors (Age 7-10)", "Middle School (Age 11-14)"].map((lvl) => (
                        <button
                          key={lvl}
                          type="button"
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">Optional Plot Twists (AI Grounding)</label>
                    <textarea 
                ...
                >
                  {isStoryLoading ? (
                    <>
                      <RefreshCw className="w-4 h-4 animate-spin" /> Gathering scroll chapters...
                    </>
                  ) : (
                    <>
                ...
                {speechSynthesisActive && (
                  <div className="pt-4 border-t border-slate-100 space-y-2">
                    <h3 className="text-xs font-bold text-slate-700 flex items-center gap-1.5">
                      <Volume2 className="w-4 h-4 text-emerald-600" /> AI Reader Voice Narration
                    </h3>
              ...
              <div className="lg:col-span-8 space-y-6">
                ...
                <div className="bg-slate-900 text-indigo-50 rounded-3xl p-6 md:p-8 min-h-[420px] flex flex-col justify-between border border-slate-800 shadow-xl relative overflow-hidden">
                  ...
                  {isStoryLoading ? (
                    <div className="flex-1 flex flex-col items-center justify-center space-y-4">
                      ...
                      <p className="text-amber-300 text-xs font-mono">The AI Storyteller is sketching your adventure...</p>
                    </div>
                  ) : currentChapter ? (
                    <div className="space-y-6 relative z-10 index">
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-indigo-950 pb-4 gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          {isNarrating ? (
                            ...
                            <button
                              onClick={() => narrateStory(currentChapter.storySegment)}
                            ...
                            >
                              <Volume2 className="w-3.5 h-3.5" /> Listen to AI Librarian
                            </button>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
                        ...
                        <div className="md:col-span-8 space-y-4 leading-relaxed font-sans text-sm md:text-base text-slate-200">
                          {currentChapter.storySegment.split("\n\n").map((para, i) => (
                            <p key={i}>
                              {para.startsWith("**") ? (
                                <span className="leading-relaxed whitespace-pre-line">{para.replace(/\*\*/g, "")}</span>
                              ) : para}
                            </p>
                          ))}
                        </div>
                        ...
                        <div className="md:col-span-4 bg-slate-800/80 p-4 rounded-2xl border border-slate-700 text-xs text-indigo-200 space-y-3">
                          ...
                          <div className="w-full aspect-[4/3] rounded-lg bg-indigo-950 flex flex-col items-center justify-center p-3 text-center border border-indigo-900 shadow-inner relative overflow-hidden">
                            ...
                            <span className="text-2xl mb-1.5 block relative z-10">📖</span>
                        ...
                        {currentChapter.isEnd ? (
                          <div className="space-y-4">
                            <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-200 text-xs">
                              <span className="font-bold text-amber-300 text-sm block mb-1">🎓 Knowledge Quest Achieved!</span>
                              Your exploration has concluded beautifully. You have completed the story pathway! Remember the morals learned from this KV reading experience.
                            </div>
                            ...
                            <div className="flex gap-4">
                              ...
                              >
                                <Bookmark className="w-4 h-4 text-amber-400" /> Save this Adventure
                              </button>
                            </div>
                          </div>
                        ) : (
                          <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {currentChapter.choices && currentChapter.choices.length > 0 ? (
                                ...
                                <p className="text-xs text-red-400">No options found. Restart your adventure configuration!</p>
                              )}
                            </div>
                          </div>
                        )}
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-6 space-y-4">
                      ...
                      <div className="space-y-1.5 max-w-sm">
                        <h3 className="text-lg font-bold text-white">Assemble Your Story Matrix</h3>
                        <p className="text-slate-400 text-xs">
                          Configure your theme on the left panel and click 'Begin Story Quest' to let the AI voice librarian craft a tailor-made adventure.
                        </p>
                      </div>
                    </div>
                  )}
                ...
                {savedStories.length > 0 && (
                  <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-4">
                    <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Your Storytelling LedgerBook</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {savedStories.map((story) => (
                        <div key={story.id} className="p-4 rounded-2xl border border-slate-100 space-y-2 text-xs relative">
                          <div className="flex justify-between items-center">
                          ...
                          <p className="text-slate-500 line-clamp-2">
                            Protagonist: **{story.character}** was selected. This story is stored internally within this laboratory node.
                          </p>
                        </div>
                      ))}
                    </div>
            ...
            >
                ...
                {bookReservationSuccess && (
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="leading-relaxed text-emerald-800">{bookReservationSuccess}</p>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <div className="bg-white rounded-3xl border border-slate-100 p-6 md:p-8 shadow-sm space-y-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div>
                    <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                      <BookOpen className="w-5 h-5 text-red-800" /> Recent Arrivals & Library Catalog
                    </h2>
                    <p className="text-xs text-slate-400">Newly cataloged books, student textbooks, and reference items at PM Shri KV IIT Powai Library.</p>
                  </div>
                  ...
                  <div className="inline-flex items-center gap-2 bg-slate-50 border border-slate-150 px-3.5 py-1.5 rounded-full text-xs font-semibold text-slate-600">
                    ...
                    <span>{recentBooksList.filter(b => b.status === "Available").length} copies ready for check-out</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2">
                  ...
                  {/* Search input */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">Search Books</label>
                  ...
                  {/* Genre filter */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">Filter by Genre</label>
                    <select
                    ...
                    >
                      ...
                      <option value="Inspirational">Inspirational / STEM</option>
                      <option value="Classic">Classic Literature / Fiction</option>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">Grade Stream</label>
                    <select
                    ...
                    >
                      ...
                      <option value="Secondary">Senior Secondary (Class 11-12)</option>
                    </select>
                  </div>

                  {/* Status filter */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">Shelving Status</label>
                    <select
                    ...
                    >
                      ...
                      <option value="Reference Only">Reference Desk Only</option>
                    </select>
                  </div>
                ...
                {(booksSearchQuery !== "" || selectedGenreFilter !== "All" || selectedGradeFilter !== "All" || selectedStatusFilter !== "All") && (
                  <div className="flex items-center justify-between text-xs bg-amber-500/5 px-4 py-2.5 rounded-xl border border-amber-500/15">
                    <span className="text-amber-800 font-semibold">Filtering is currently active ({filteredRecentBooks.length} matched outcomes)</span>
                    <button
                      onClick={() => {
                        setBooksSearchQuery("");
              ...
              {filteredRecentBooks.length > 0 ? (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={`h-3 w-full ${
                          book.genre.includes("STEM") ? "bg-purple-600" :
                          book.genre.includes("Classic") ? "bg-emerald-600" :
                          book.genre.includes("Biography") ? "bg-cyan-600" :
                          book.genre.includes("Cosmology") ? "bg-slate-800" :
                          "bg-amber-500"
                        }`} />
                        ...
                        <div className="p-6 space-y-4 flex-1">
                          <div className="flex justify-between items-start gap-2">
                            ...
                            <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${
                              book.status === "Available" ? "bg-emerald-50 text-emerald-800 border border-emerald-200" :
                              book.status === "Reserved" ? "bg-amber-50 text-amber-800 border border-amber-200" :
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500 font-serif mt-1">Written by <strong className="text-slate-700">{book.author}</strong></p>
                          </div>

                          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-slate-400 font-mono">
                            <span>📚 Pages: <strong className="text-slate-600">{book.pages}</strong></span>
                            <span>📍 Location: <strong className="text-slate-600">{book.rackLocation}</strong></span>
                            <span>🎯 Grade: <strong className="text-slate-600">{book.gradeLevel}</strong></span>
                          </div>

                          <p className="text-slate-600 text-xs leading-relaxed line-clamp-3">{book.description}</p>
                          
                          {/* Why Recommended box */}
                          <div className="p-3 bg-red-50/40 border border-red-100/50 rounded-2xl text-[11px] text-red-950">
                            <span className="font-bold text-red-800 block mb-1">🎓 NEP RECOMMENDATION CRITERIA:</span>
                            {book.whyRecommended}
                          </div>
                          ...
                          <div className="p-3 bg-amber-50/30 border border-amber-100/50 rounded-2xl text-[11px] text-amber-950 space-y-1">
                            <span className="font-bold text-amber-800 flex items-center gap-1">
                              <Award className="w-3.5 h-3.5 text-amber-500" /> KV HANDS-ON CHALLENGE:
                            </span>
                            <p className="italic leading-relaxed text-slate-600">{book.funActivity}</p>
                          </div>
                        </div>
                        ...
                        <div className="bg-slate-50 border-t border-slate-100 px-6 py-4 flex items-center justify-between gap-4">
                          {book.status === "Available" ? (
                            ...
                            >
                              <BookOpenCheck className="w-4 h-4" /> Reserve To Borrow
                            </button>
                          ) : (
                            <button
                              disabled
                            ...
                            >
                              {book.status === "Reserved" ? "Copy Reserved" : "Reference Only"}
                            </button>
                          )}
                ...
                <div className="bg-slate-50 rounded-3xl border border-dashed border-slate-200 p-12 text-center max-w-lg mx-auto space-y-4">
                  ...
                  <div>
                    <h3 className="text-slate-800 font-bold">No Match in Recent Books Shelf</h3>
                    <p className="text-slate-500 text-xs">
                      We couldn't locate any recent books matching your custom filter configuration. Try adjusting your grade level, shelving status, or typing terms in the search bar.
                    </p>
                  </div>
                  <button
                    onClick={() => {
          ...
          {activeTab === "creative" && (
            ...
            >
              ...
              <div className="lg:col-span-5 bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-6">
                <div>
                  <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                    <PenTool className="w-5 h-5 text-purple-600" /> KV Student Creative Desk
                  </h2>
                  <p className="text-xs text-slate-400">Collaborate with the AI literary critic to compose poems, lyrics, or novels.</p>
                </div>

                <div className="space-y-4">
                  {/* Lit topic */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">Literary Subject / Topic</label>
                    <input 
                      type="text" 
                      value={creativeTopic}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">Literary Format</label>
                    <select
                      value={creativeForm}
                  ...
                  {/* Literary Mood */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">Mood & Expression</label>
                    <select
                      value={creativeMood}
                  ...
                  {/* Keywords tag cloud */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5">Keywords to blend in (comma filtered)</label>
                    <input 
                      type="text" 
                    ...
                    />
                    <span className="text-[10px] text-slate-400 mt-1 block">Separate terms by commas so the AI extracts them safely.</span>
                  </div>
                </div>
                ...
                >
                  {isCreativeLoading ? (
                    <>
                      <RefreshCw className="w-4 h-4 animate-spin" /> Mentoring literary devices...
                    </>
                  ) : (
                    <>
                      <Sparkles className="w-4 h-4 text-purple-200" /> Generate Beautiful Masterpiece
                    </>
                  )}
                </button>
              </div>
              ...
              <div className="lg:col-span-7 space-y-6">
                ...
                {isCreativeLoading ? (
                  <div className="bg-slate-900 text-white rounded-3xl p-12 text-center h-full min-h-[380px] flex flex-col items-center justify-center space-y-4 border border-indigo-950">
                    ...
                    <p className="text-purple-300 text-xs font-mono">Blending syllables and rhyme structures...</p>
                  </div>
                ) : creativeResult ? (
                  <div className="bg-white rounded-3xl border border-slate-100 p-6 md:p-8 shadow-sm space-y-6">
                    ...
                    <div className="border-b border-purple-100 pb-4">
                      ...
                      <h3 className="text-xl font-bold font-sans text-slate-800 mt-2">{creativeResult.title}</h3>
                    </div>
                    ...
                    <div className="space-y-3 pt-4 border-t border-slate-100">
                      <h4 className="text-xs font-bold text-slate-700 uppercase tracking-widest flex items-center gap-1.5">
                        <Award className="w-4 h-4 text-amber-500" /> Mentor insights & Literary Devices
                      </h4>
                      <p className="text-xs text-slate-400">
                        Ashish Kumar’s library guide explains how to replicate these artistic techniques in standard CBSE tests:
                      </p>
                      
                      <div className="space-y-2">
                        {creativeResult.educationalTips && creativeResult.educationalTips.map((tip, idx) => (
                          <div key={idx} className="flex gap-2.5 items-start text-xs bg-slate-50 p-3 rounded-xl border border-slate-100">
                            ...
                            <span className="text-slate-600 leading-relaxed font-sans">{tip}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="flex gap-4">
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(`${creativeResult.title}\n\n${creativeResult.output}`);
                          alert("Poem copied onto clipboard! Show it to your class tutor.");
                        }}
                  ...
                  <div className="bg-slate-900 text-white rounded-3xl p-8 text-center min-h-[380px] flex flex-col justify-center items-center space-y-4">
                    ...
                    <div className="space-y-1.5 max-w-sm">
                      <h3 className="text-lg font-bold">Write like a Nobel Laureate</h3>
                      <p className="text-slate-400 text-xs">
                        Configure a topic or write about Sanjay Gandhi National Park, and watch the AI write and breakdown the classical literary elements used!
                      </p>
                    </div>
                  </div>
                )}
            ...
            >
              ...
              <div className="lg:col-span-5 space-y-6">
                <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-6">
                  <div>
                    <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                      <MessageSquare className="w-5 h-5 text-red-800" /> Share Your Reading Journey
                    </h3>
                    <p className="text-xs text-slate-400">Post reviews or library stories on our simulated KV Powai Library wall!</p>
                  </div>
                ...
                <form onSubmit={handleAddPost} className="space-y-4">
                  ...
                  {/* Rating Selector */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">My Star Rating</label>
                      ...
                      {[1, 2, 3, 4, 5].map((star) => (
                        <button
                          ...
                          className={`text-xl transition-transform hover:scale-110 ${
                            newPostRating >= star ? "text-amber-500" : "text-slate-200"
                          }`}
                        >
                          ★
                        </button>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">Hashtags (Comma separated)</label>
                    <input 
                      type="text" 
                      value={newPostTags}
                  ...
                  {/* Post Content */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">Write your review / reflection *</label>
                    <textarea 
                      required
              ...
              <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-6">
                <div>
                  ...
                  <h3 className="text-base font-bold text-slate-800 mt-2.5 flex items-center gap-1.5">
                    <Radio className="w-4 h-4 text-red-800 animate-pulse" /> Official Social Media Hub
                  </h3>
                  <p className="text-xs text-slate-400 mt-1">Get physical meet updates, book delivery schedules, and notifications from Kendriya Vidyalaya.</p>
                </div>
                ...
                <AnimatePresence>
                  {socialAlertMessage && (
                    ...
                    >
                      ...
                      <span className="leading-relaxed">{socialAlertMessage}</span>
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-1">
                          <span className="font-bold text-xs text-slate-800">Instagram Handle</span>
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" title="Verified link"></span>
                        </div>
                        <span className="text-[11px] text-slate-500 block">@pmshri_kviitpowai_lib</span>
                        <span className="text-[10px] text-indigo-900/60 font-medium">1,420 Active Readers</span>
                      </div>
                    </div>
                    <button
                      onClick={() => handleToggleFollow("Instagram", "@pmshri_kviitpowai_lib")}
                  ...
                  <div className="p-4 bg-gradient-to-r from-red-50/50 to-rose-50/20 border border-slate-100 rounded-2xl flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-1">
                          <span className="font-bold text-xs text-slate-800">YouTube Channel</span>
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                        </div>
                        <a 
                        ...
                        >
                          Library Point <ExternalLink className="w-2.5 h-2.5 inline text-red-500" />
                        </a>
                      </div>
                    </div>
                    <button
                      onClick={() => handleToggleFollow("YouTube", "Library Point")}
                      className={`px-3 py-1.5 rounded-xl text-[11px] font-semibold transition-all border ${
                    ...
                    >
                      {followedHandles["YouTube"] ? "Subscribed" : "Subscribe"}
                    </button>
                  </div>
                  ...
                  <div className="p-4 bg-gradient-to-r from-sky-50/50 to-blue-50/20 border border-slate-100 rounded-2xl flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-1">
                          <span className="font-bold text-xs text-slate-800">X Desk Feed</span>
                          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                        </div>
                        <span className="text-[11px] text-slate-500 block">@KVIITPowaiLib</span>
                        <span className="text-[10px] text-indigo-900/60 font-medium">412 Followers</span>
                      </div>
                    </div>
                    <button
                      onClick={() => handleToggleFollow("X", "@KVIITPowaiLib")}
                      className={`px-3 py-1.5 rounded-xl text-[11px] font-semibold transition-all border ${
                        followedHandles["X"]
                          ? "bg-emerald-50 border-emerald-200 text-emerald-800"
                          : "bg-slate-900 hover:bg-slate-800 border-transparent text-white"
                  ...
                  <div className="p-4 bg-gradient-to-r from-blue-50/50 to-indigo-50/20 border border-slate-100 rounded-2xl flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-1">
                          <span className="font-bold text-xs text-slate-800">Facebook Club</span>
                        </div>
                        <span className="text-[11px] text-slate-500 block">KV IIT Powai Readers Group</span>
                        <span className="text-[10px] text-indigo-900/60 font-medium">2,300 Members</span>
                      </div>
                    </div>
                    <button
                      onClick={() => handleToggleFollow("Facebook", "KV IIT Powai Readers")}
                      className={`px-3 py-1.5 rounded-xl text-[11px] font-semibold transition-all border ${
              ...
              <div className="lg:col-span-7 space-y-6">
                ...
                <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                  <h3 className="text-sm font-bold text-slate-800 uppercase tracking-widest">Library Buzz Feed</h3>
                  <span className="text-xs text-slate-400 font-mono italic">Real-Time Community Updates</span>
                </div>
                ...
                <div className="space-y-4">
                  {socialPosts.length === 0 ? (
                    <div className="bg-white rounded-3xl border border-slate-100 p-8 text-center space-y-4 shadow-sm">
                      ...
                      <div className="space-y-1">
                        <h4 className="text-sm font-bold text-slate-800">No Posts on the Wall Yet</h4>
                        <p className="text-xs text-slate-400 max-w-sm mx-auto">
                          Be the first to share your reading journey on the official school library wall! Post a book review or library story using the form on the left.
                        </p>
                      </div>
                    </div>
                  ) : (
                    socialPosts.map((post) => {
                      ...
                      return (
                        <div key={post.id} className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-4">
                            ...
                            <div className="flex gap-3">
                              ...
                              <div>
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  <h4 className="font-bold text-xs text-slate-800">{post.studentName}</h4>
                                </div>
                                <p className="text-[10px] text-slate-400 font-mono mt-0.5">{post.timestamp}</p>
                              </div>
                            </div>
                          ...
                          <div className="space-y-2">
                            <div className="p-3 bg-slate-50 border border-slate-100 rounded-xl">
                              ...
                              <span id="book-title-social" className="font-sans font-bold text-slate-800 text-sm">
                                {post.bookTitle}
                              </span>
                              {post.author && (
                          ...
                          <div className="flex flex-wrap gap-1.5 pt-2">
                            {post.tags && post.tags.map((tag, i) => (
                              <span key={i} className="text-[9px] font-medium text-red-800 bg-red-50 px-2 py-0.5 rounded-full border border-red-100/50">
                                #{tag}
                              </span>
                            ))}
                          </div>
                          ...
                          <div className="flex items-center gap-4 pt-3 border-t border-slate-50 text-slate-400 text-xs">
                            ...
                            >
                              <Heart className={`w-4 h-4 ${hasLiked ? "fill-rose-600 text-rose-600" : "group-hover:scale-110"}`} />
                            ...
                            {commsList.length > 0 && (
                              <div className="space-y-2">
                                {commsList.map((commText, idx) => (
                                  <div key={idx} className="text-xs p-2 bg-white rounded-lg border border-slate-100 leading-relaxed text-slate-600">
                                    {commText}
                                  </div>
                                ))}
                              </div>
      ...
      <footer className="bg-slate-900 border-t border-indigo-950 px-4 md:px-8 py-6 text-slate-400 text-xs">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="text-center md:text-left space-y-1">
            <h4 className="text-white font-bold leading-none">PM Shri Kendriya Vidyalaya IIT Powai Sector</h4>
            <p className="text-[10px] text-slate-500">Autonomous body under Ministry of Education, Government of India</p>
          </div>
          
          <div className="flex flex-wrap gap-4 text-[10px] text-slate-400 font-semibold justify-center">
            <a href="#welcome" onClick={(e) => { e.preventDefault(); setActiveTab("dashboard"); }} className="hover:text-amber-400">Library Samhita</a>
            <span>•</span>
            <a href="#ai-ethics" onClick={(e) => { e.preventDefault(); alert("AI interaction behaves responsibly under NEP 2020 frameworks. No student PII data is transmitted."); }} className="hover:text-amber-400">AI Safety Guidelines</a>
            <span>•</span>
            <a href="#help" onClick={(e) => { e.preventDefault(); alert("Contact Librarian via official mail: ashishkumar.librarian@gmail.com"); }} className="hover:text-amber-400">Ask Librarian Support</a>
          </div>
        </div>
      </footer>
